        ...
        protected override void End()
        {
            ...
            if (!(BoundParameters.ContainsKey(nameof(Path)) || BoundParameters.ContainsKey(nameof(Path64))))
            {
                ThrowTerminatingError(new ErrorRecord(
                    new NotSupportedException("Parameters are incorrect; either -Path or -Path64 must be specified."),
                    $"{ErrorId}.InvalidParameters",
                    ErrorCategory.InvalidArgument,
                    targetObject: null));
            }

            var helper = new SevenZipHelper(this, PipelineStopToken);
            helper.ExtractFiles(Path, Path64, PackageName, Destination, FilesToExtract, DisableLogging);
