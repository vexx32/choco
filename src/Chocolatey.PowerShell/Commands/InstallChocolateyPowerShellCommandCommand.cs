...
namespace Chocolatey.PowerShell.Commands
{
    ...
    public class InstallChocolateyPowerShellCommandCommand : ChocolateyCmdlet
    {
        /*
        .SYNOPSIS
        ...
        */
        private const string BatchScript = @"@echo off
powershell -NoProfile -ExecutionPolicy unrestricted -Command ""& '{0}'  %*""";

        [Parameter(Position = 0)]
        public string PackageName { get; set; }

        [Parameter(Mandatory = true, Position = 1)]
        [Alias("PSFileFullPath", "FileFullPath", "File")]
        ...
        public string Url64Bit { get; set; } = string.Empty;
        ...
        protected override void End()
        {
            ...
            {
                // TODO: Call into GetChocolateyWebFile
            }

            var chocolateyPackageName = EnvironmentVariable(EnvironmentVariables.ChocolateyPackageName);
            if (!string.IsNullOrEmpty(chocolateyPackageName) && chocolateyPackageName == EnvironmentVariable(EnvironmentVariables.ChocolateyInstallDirectoryPackage))
            {
                WriteWarning("Install Directory override not available for PowerShell command packages.");
            }

            var binPath = CombinePaths(this,ChocolateyInstallLocation, "bin");

            var fullPath = GetFullPath(this,Path);
            var commandName = System.IO.Path.GetFileNameWithoutExtension(fullPath);
            var batchFileName = CombinePaths(this,binPath, $"{commandName}.bat");

            WriteHost($"Adding {batchFileName} and pointing it to powershell command {fullPath}");
            SetContent(this, batchFileName, string.Format(BatchScript, fullPath), Encoding.ASCII);
        }
    }
}
