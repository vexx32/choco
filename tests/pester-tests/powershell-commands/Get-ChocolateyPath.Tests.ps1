Describe 'Get-ChocolateyPath helper function tests' -Tags Cmdlets, GetChocolateyPath {
    ...
    Context 'Invalid PathType' {
        It 'Throws a ParameterBindingException' {
            { Get-ChocolateyPath -PathType InvalidPath } | Should -Throw -ExceptionType 'System.Management.Automation.ParameterBindingException'
        }
    }
}